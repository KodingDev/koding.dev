...
const Wordmark: React.FC<WordmarkProps> = ({ client }) => {
  if (client.wordmark) {
    return <Image src={client.wordmark} alt={`${client.name} logo`} className="mb-0.5 inline-block h-3.5 w-auto" />;
  }

  if (client.avatar) {
    return (
      <span className="font-medium">
        <Image src={client.avatar} alt={`${client.name} logo`} className="mr-1 mb-0.5 inline-block size-4" />
        {client.name}
      </span>
    );
  }
    ...
    <div className="flex flex-col gap-16 pb-36">
      ...
      <div className="relative w-fit py-8">
        ...
        <div className="flex flex-col gap-3">
          <p className="font-serif text-6xl">Stella Inwood</p>
          <h1 className="text-xl leading-[1.8] sm:leading-[1.8]">
            {/* TODO: Counting up */}
            Crafting rich, engaging gameplay experiences for{" "}
            <span className="whitespace-nowrap rounded-xl bg-primary px-2 py-1 text-primary-foreground">8 years</span>
          </h1>
        </div>
        ...
        <div className="hidden sm:block">
          ...
          {/* Highlight - Blue Star */}
          <div className="-left-8 absolute top-8">
            <TwinklingStar>
          ...
          <div className="-left-14 absolute bottom-12">
            <TwinklingStar>
              <svg width="50" height="50" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
                <title>Star</title>
                <path
          ...
          <div className="-right-8 absolute bottom-16">
            <TwinklingStar>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(300px,1fr)_minmax(300px,1fr)_400px] lg:grid-rows-[1fr_auto]">
        {/* Main intro image - spans 2 rows on desktop, full width on mobile */}
        <div className="flex flex-col gap-2 rounded-md border p-2 lg:col-span-2 lg:row-span-2">
          {/* TODO: More polaroid-y? */}
          <Image src={StellaIntro} alt="Person working on a computer" className="aspect-4/3 w-full rounded-md" />
          {/*<p>*/}
          {/*  Non exercitation excepteur non labore eu ullamco eiusmod qui aliqua mollit magna irure laboris voluptate*/}
          {/*  anim. Velit minim ullamco aliqua ipsum aute deserunt aliquip incididunt labore amet ipsum. Pariatur Lorem*/}
          {/*  veniam labore sint irure officia non tempor duis duis duis. Do nisi minim nisi irure incididunt incididunt*/}
          {/*  enim Lorem. Labore do incididunt non ea. Tempor proident ut nostrud veniam fugiat excepteur pariatur enim.*/}
          {/*</p>*/}
        </div>

        {/* Clients section - right column top */}
        <div className="flex flex-col justify-between gap-2 rounded-md border p-2">
          <div>
            <h1 className="font-serif text-xl uppercase">Clients</h1>
            <p className="text-sm">The wonderful people I&apos;ve had the privilege to work for.</p>
          </div>
          <div className="flex flex-col *:not-last:border-b">
            {allClients.slice(0, 5).map((client) => (
              <p key={client.slug} className="py-1 text-muted-foreground text-sm">
                <Wordmark client={client} /> / {client.role} / {client.start} - {client.end ?? "Present"}
              </p>
            ))}
          </div>
        </div>

        {/* Hazel cat section - right column bottom */}
        <div className="flex flex-col gap-2 rounded-md border p-2">
          {/* TODO: More polaroid-y? */}
          <Image src={Hazel} alt="Cat" className="h-64 w-full rounded-md object-cover" />
          <p className="text-muted-foreground text-sm">My beautiful cat Hazel (mraow!)</p>
        </div>
      </div>

      {/* Projects */}
      <div>
        <div className="flex flex-col justify-between gap-4 pb-8 align-middle md:flex-row md:items-center">
