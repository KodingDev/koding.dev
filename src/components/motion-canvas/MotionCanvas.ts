/**
 * @license MIT
 * @see https://github.com/motion-canvas/motion-canvas/blob/main/packages/player/src/main.ts
 */
...
class MotionCanvasPlayer extends HTMLElement {
  ...
  private set auto(value: boolean) {
    this.setAttribute('auto', value ? 'true' : 'false');
  }
  ...
  private readonly button: HTMLDivElement | null;

  private state = State.Initial;
  private project: Project | null = null;
  private player: Player | null = null;
  private defaultSettings: (PlayerSettings & StageSettings) | null = null;
  ...
  private async updateSource(source: string) {
    ...
    let project: Project;
    try {
      const promise = import(/* webpackIgnore: true */ /* @vite-ignore */ source);
  ...
  private attributeChangedCallback(name: string, _: never, newValue: never) {
    switch (name) {
      case 'auto':
        this.setPlaying(this.playing);
        break;
  ...
  private updateSettings() {
    ...
    if (!width || !height || !quality || !this.defaultSettings) return;
