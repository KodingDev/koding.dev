...
export const ProjectRow: React.FC<ProjectRowProps> = ({ project, featured }) => {
  ...
  return (
    ...
    >
      ...
      <span className="ml-auto shrink-0 text-sm text-muted-foreground">{project.year ?? "Current"}</span>

      <MdArrowOutward className="relative top-0.5 size-4 shrink-0 text-muted-foreground transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
    </Link>
  );
};
